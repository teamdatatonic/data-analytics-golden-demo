...
####################################################################################
airflow_data_path_to_tf_script = "/home/airflow/gcs/data/terraform/workflows"
dag_prefix_name = "sample-terraform-workflows"
auto_delete_hours = 1 # set to zero to never delete
terraform_bash_file = "sample_terraform_workflows.sh"
...
workflow_name             = "sample-workflow-" + os.environ['ENV_RANDOM_EXTENSION']
workflow_region           = os.environ['ENV_DATAFORM_REGION']
workflow_trigger_bucket   = os.environ['ENV_DATAPROC_BUCKET']
...
params_list = { 
    ...
    'workflow_trigger_bucket'        : workflow_trigger_bucket,
...
def delete_environment(deploy_or_destroy, is_manual_run):
    print("BEGIN: delete_environment")
    is_manual_run = bool(strtobool(is_manual_run))
    ...
    if deploy_or_destroy == "destroy":
        ...
        if os.path.exists(filePath):
            ...
            # If DAG was scheduled manually we will always proceed with environment cleanup
            if is_manual_run:
                print("Manual DAG trigger detected. Deleting Environment")
                delete_environment = True
            # Test for auto_delete_hours hours
            elif auto_delete_hours == 0:
...
with airflow.DAG(dag_display_name,
    ...
    delete_environment = PythonOperator(
        ...
        op_kwargs={
            "deploy_or_destroy": is_deploy_or_destroy,
            "is_manual_run": "{{ dag_run.external_trigger }}"
        },
        execution_timeout=timedelta(minutes=1),
        dag=dag,
        ) 
