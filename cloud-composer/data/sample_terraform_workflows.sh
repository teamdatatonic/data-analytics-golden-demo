...
terraform apply {{ params.terraform_destroy }}  -auto-approve \
  ...
  -var="workflow_trigger_bucket={{ params.workflow_trigger_bucket }}" \
  -var="project_number={{ params.project_number }}" \
  -var="workflow_name={{ params.workflow_name }}" \
  -var="workflow_region={{ params.workflow_region }}"
'  
